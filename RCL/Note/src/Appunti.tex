...
\pagebreak
	...
	Concretamente, DNS \`e un meccanismo che specifica la sintassi dei nomi e le regole per gestirli, consente la conversioni da nomi a indirizzi e viceversa.
	\`E costituito da:
	\vspace{-3.5mm}
	\begin{itemize}
		\itemsep-0.3em
		\item \textbf{Schema di assegnazione dei nomi} gerarchico e basato su domini.
		\item \textbf{Database distribuito} contenente i nomi e le corrispondenze con gli indirizzi IP implementato con una gerarchia di name server.
	...
	Lo spazio dei domini ha una struttura ad albero; ogni nodo dell'albero dello spazio dei domini ha un nome di dominio, ossia una sequenza di diverse
	etichette separate da punti (e.g. ``lab3''.``di''.``unipi''.``it'').
