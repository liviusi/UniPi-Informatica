...
\begin{itemize}[topsep=0pt, itemsep=0pt, parsep=0pt]
	...
	\begin{itemize}[topsep=0pt, itemsep=0pt, parsep=0pt]
		...
		\begin{itemize}[topsep=0pt, itemsep=0pt, parsep=0pt]
			...
			Nel caso di segmenti fuori sequenza, i dati possono essere temporaneamente memorizzati dall’entit\`a TCP destinataria (TCP non dice come il destinatario
			deve gestire i pacchetti fuori \linebreak sequenza); nelle versioni più recenti si implementa ``SACK": i pacchetti ricevuti fuori sequenza vengono memorizzati
			e il riscontro di pacchetti fuori sequenza e duplicati viene inviato in OPTIONS.
		...
		Per alcuni esempi, si vedano le figure \ref{fig:tcp-caso-ok}, \ref{fig:tcp-riscontro-smarrito}, \ref{fig:tcp-segmento-smarrito},
		\ref{fig:tcp-riscontro-smarrito-ritrasmissione}.
		...
		Per alcuni esempi, si vedano le figure \ref{fig:chiusura-3way}, \ref{fig:tcp-chiusura}, \ref{fig:tcp-apertura-chiusura}.
	\end{itemize}
