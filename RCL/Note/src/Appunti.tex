...
\begin{itemize}[topsep=0pt, itemsep=0pt, parsep=0pt]
	...
	\begin{itemize}[topsep=0pt, itemsep=0pt, parsep=0pt]
		...
		\begin{itemize}[topsep=0pt, itemsep=0pt, parsep=0pt]
			...
			deve gestire i pacchetti fuori sequenza) temporaneamente; nelle versioni più recenti si implementa ``SACK": i pacchetti ricevuti fuori sequenza vengono memorizzati
		...
		incapsulato in un pacchetto IP); l'algoritmo utilizzato \`e costituito da tre passi:
		\begin{itemize}
			\item \textbf{Partenza lenta} (o \textbf{slow start}):
			...
			ogni ACK ricevuto in modo tale da avere per ogni RTT una crescita pari a 1MSS; la dimezza a ogni evento di perdita.
			% Si veda la figura \ref{fig:tcp-aimd}.
			\item \textbf{Ripresa veloce} (o \textbf{fast recovery}):
			\item \textbf{Reazione ai timeout}:
		\end{itemize}

		Di fatto, la dimensione della finestra di invio \`e il minimo tra rwnd e cwnd.
